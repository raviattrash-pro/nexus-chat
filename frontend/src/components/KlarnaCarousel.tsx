...
const mkItem = ([src, label, id]: [string, string, string]): CarouselItem => ({
    id,
    image: { src },
    buttonImage: { src },
    label,
...
export default function KlarnaCarousel(props: KlarnaCarouselProps) {
    ...
    const select = useCallback(
        (itemIdx: number) => {
            ...
            if (itemIdx === currentActive) {
                if (onSelect) onSelect(list[itemIdx]);
                return;
            }
    ...
    return (
        ...
        >
            <div
                ...
                onClick={() => { if(onSelect) onSelect(list[active]) }}
            >
                        ...
                        {srcOf(list[active]?.image) && (
                            <img
                                ...
                                style={{
                                    ...
                                    objectFit: "cover",
                                    display: "block",
                                    borderRadius: cardRadiusPx ? `${cardRadiusPx}px` : "16px"
                                }}
                            />
                        )}
                    </motion.div>
